...
//-----------------------------------------------------------------------------
contract ToyCreation is Ownable, ToyTransfers, ToyInterfaceSupport {
    //-------------------------------------------------------------------------
    ...
    PlayInterface play = PlayInterface(0xD51106F4610055dF9255c8E6c0d708477015ce4e);
    ...
    /// @dev Throws if TOY Token #`_toyId` does not exist. Throws if TOY Token
    ///  is not owned by sender. Throws if '_toyId' is smaller than 8 bytes.
    ///  Throws if '_newUid' is bigger than 7 bytes. Throws if '_newUid' is
    ///  zero. Throws if '_newUid' is already taken.
    ...
    function link(
        ...
        uint _toyId, 
        bytes _data
    ) external mustExist(_toyId) mustOwn(_toyId) {
        ...
        require (_newUid > 0 && uint(_newUid) < 0xFFFFFFFFFFFFFF, "Invalid new UID");
    ...
    ///  is not owned by sender. Throws if '_toyId' is smaller than 8 bytes.
    ///  Throws if '_newUid' is bigger than 7 bytes. Throws if '_newUid' is
    ///  zero. Throws if '_newUid' is already taken. Throws if sender is not
    ///  the approved address of TOY Token #`from` or an authorized operator
    ///  for `to`.
    /// @param _owner The owner of TOY Token #`_toyId`. Sender must be approved
    /// @param _newUid The UID of the RFID chip to link to the TOY Token
    /// @param _toyId The UID of the empty TOY Token to link
    /// @param _data A byte string of data to attach to the TOY Token
    //-------------------------------------------------------------------------
    function linkFrom(
        ...
        uint _toyId, 
        bytes _data
    ) external mustExist(_toyId) {
        ToyToken storage toy = toyArray[uidToToyIndex[_toyId]];
        // sender must own TOY Token
        require (_owner == toy.owner, "TOY Token not owned by '_owner'");
        // _toyId must be an empty TOY Token
        require (_toyId > uidBuffer, "TOY Token already linked");
        // _newUid field cannot be empty or greater than 7 bytes
        require (_newUid > 0 && uint(_newUid) < 0xFFFFFFFFFFFFFF, "Invalid new UID");
        ...
        require (
            msg.sender == idToApprovedAddress[_toyId] ||
            operatorApprovals[_owner][msg.sender] == true,
            "Not authorized to link this TOY Token"
        );
        // set new UID's mapping to index to old UID's mapping
        uidToToyIndex[uint(_newUid)] = uidToToyIndex[_toyId];
        // reset old UID's mapping to index
